...
import (
	...
	"github.com/kodergarten/pgtestdb"
	"github.com/kodergarten/pgtestdb/migrators/dbmatemigrator"
)
...
func TestDbmateMigratorWithFS(t *testing.T) {
	...
	var numMigrations int
	err = db.QueryRowContext(ctx, "select count(*) from schema_migrations").Scan(&numMigrations)
	assert.Nil(t, err)
	check.Equal(t, 3, numMigrations)
